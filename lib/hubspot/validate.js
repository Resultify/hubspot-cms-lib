...
async function checkHubspotValidationStatus (accountId, validationId, spinner) {
  try {
    const checkValidationStatus = async () => {
      ...
      if (validationStatus.data === 'REQUESTED') {
    ...
    if (status === 'FAIL') {
      ...
      if (childTheme) {
        failedValidations = failedValidations.filter((/** @type {any} */ item) =>
          ...
          item.check !== 'module_has_non_default_label' &&
          item.check !== 'restricted_template_type')
        warningValidations = warningValidations.filter((/** @type {any} */ item) =>
          item.check !== 'checks_lighthouse_scores_present' &&
...
async function marketplaceValidate (config, themeName) {
  try {
    ...
    let validationId = await requestHubspotValidation(portalId, themeName, spinner)
    ...
    let validationResults = await fetchHubspotValidationResults(portalId, validationId)
