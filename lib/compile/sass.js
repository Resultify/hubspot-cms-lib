...
async function compileScss (opt) {
  ...
  try {
    ...
    await fsPromises.mkdir(globals.SCSS_DIST, { recursive: true })
    ...
    if (files !== undefined) {
      for await (const file of files) {
        ...
        await fsPromises.writeFile(`${globals.SCSS_DIST}/${fileName}`, scssResult.css)
        const fileStats = await fsPromises.stat(`${globals.SCSS_DIST}/${fileName}`)
        file.size = utils.convertFileSize(fileStats.size)
        file.dist = `${globals.SCSS_DIST}/${fileName}`
        fileList.push(file)
      }
    }
