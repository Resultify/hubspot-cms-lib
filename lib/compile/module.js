 ...
 * @property {Array<any>} newModuleFields - module fields data
  ...
  if (configFiles !== undefined) {
    for await (const file of configFiles) {
      if (await isFileDir(file.path)) {
        ...
        const moduleSrcName = file.name.replace('.js', '').replace('compile-', '')
        ...
        if (await isFileDir(moduleSrcPath)) {
          ...
          const moduleConfig = {
            ...
            newModuleFields: data.default.fields,
...
/**
 * #### update module fields json data
 * @async
 * @private
 * @param {StandaloneModulesConfig} config - module config
 * @returns undefined
 */
async function updateModuleFields (config) {
  const existingFields = await fsPromises.readFile(`${config.newModulePath}/fields.json`, 'utf8')
  if (existingFields !== undefined) {
    let existingFieldsObj = JSON.parse(existingFields || '{}')
    const newModuleFields = config.newModuleFields
    // let updatedFields = JSON.stringify(existingFieldsObj, null, 2)

    for (let field of existingFieldsObj) {
      for (const newField of newModuleFields) {
        if (field.name === newField.name) {
          field = { ...field, ...newField }
          // @ts-ignore
          existingFieldsObj = existingFieldsObj.map(f => f.name === field.name ? field : f)
        }
      }
      if (field.children && Array.isArray(field.children)) {
        for (let child of field.children) {
          for (const newField of newModuleFields) {
            if (child.name === newField.name) {
              child = { ...child, ...newField }
              // @ts-ignore
              field.children = field.children.map(c => c.name === child.name ? child : c)
            }
          }
          if (child.children && Array.isArray(child.children)) {
            for (let grandChild of child.children) {
              for (const newField of newModuleFields) {
                if (grandChild.name === newField.name) {
                  grandChild = { ...grandChild, ...newField }
                  // @ts-ignore
                  child.children = child.children.map(c => c.name === grandChild.name ? grandChild : c)
                }
              }
            }
          }
    ...
    const updatedFields = JSON.stringify(existingFieldsObj, null, 2)
    await fsPromises.writeFile(`${config.newModulePath}/fields.json`, updatedFields, 'utf8')
  }
}
...
async function compileModule (hideStatus) {
  try {
    ...
    const moduleSrcConfigFiles = await getFileList(`${process.cwd()}/standalone-modules/compile-*.module.js`, { objectMode: true })
    ...
    for await (const [index, module] of config.entries()) {
      ...
      await updateModuleFields(module)
